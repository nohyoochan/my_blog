<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>ROLEX 공식 판매점 - 대한민국</title>
  <link rel="stylesheet" href="https://unpkg.com/swiper/swiper-bundle.css">
  <link rel="stylesheet" href="https://unpkg.com/swiper/swiper-bundle.min.css">
  <link
    href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700;900&family=Roboto:wght@400;700;900&display=swap"
    rel="stylesheet">
  <link rel="stylesheet" href="/pf/Rolex/index.css">
  <link rel="icon" href="https://nohyoochan.github.io/img1/pf/pf1/logo2.png">
  <!-- 구글 폰트 불러오기 -->
  <!-- rotobo(400/700/900), notosanskr(400/600/900) -->
  <link
    href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR:wght@400;700;900&family=Roboto:wght@400;700;900&display=swap"
    rel="stylesheet">
  <!-- 폰트어썸 불러오기 -->
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.1/css/all.min.css">
  <link rel="stylesheet" href="../package/swiper-bundle.min.css">
  <!-- 제이쿼리 불러오기 -->
  <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
  <script src="https://unpkg.com/swiper/swiper-bundle.js"></script>
  <script src="https://unpkg.com/swiper/swiper-bundle.min.js"></script>>
  <script src="https://polyfill.io/v3/polyfill.min.js?features=default"></script>
  <script src="../package/swiper-bundle.min.js"></script>
  <!-- 구글 맵 -->
  <script async defer
    src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCDL7Ri4QVwConWr55jq1oUo5Ad_1c1L4o&callback=initMap"></script>
    <script src="https://polyfill.io/v3/polyfill.min.js?features=default"></script>
    <script
      src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCDL7Ri4QVwConWr55jq1oUo5Ad_1c1L4o&callback=initMap&libraries=&v=weekly"
      defer
    ></script>
  <script src="/pf/Rolex/index.js"></script>
</head>

<body>
  <div class="top-bar">
    <nav class="menu-box">
      <a href="#" class="logo">
        <img src="https://nohyoochan.github.io/img1/pf/pf1/logo.png" alt="" style="width:130px; height:70px;">
        <img src="https://nohyoochan.github.io/img1/pf/pf1/logo2.png" alt="" style="width:100px; height:70px;">
      </a>
      <ul>
        <div class="inner flex flex-jc-c">
          <li>
            <a href="#" class="flex flex-ai-c height-100p">Collection</a>
            <div>
              <div class="sub-box">
                <div class="left-panel">
                <a href="#">
                <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-5.png" alt="" style="width:250px; height:250px;">
                </a>
                <a href="#">
                <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-5.png" alt="" style="width:250px; height:250px;">
                </a>
                <a href="#">
                <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-5.png" alt="" style="width:250px; height:250px;">
                </a>
                <a href="#">
                <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-5.png" alt="" style="width:250px; height:250px;">
                </a>
                </div>
                <div class="right-panel">
                <a href="#">123
                <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-5.png" alt="" style="width:250px; height:250px;">
                </a>
                </div>
              </div>          
        </div>
          <li>
            <a href="#" class="flex flex-ai-c height-100p">Rolex And Partners</a>
            <div>
              <div class="sub-box">

              </div>
            </div>
          <li>
            <a href="#" class="flex flex-ai-c height-100p">Service</a>
            <div>
              <div class="sub-box">
                
              </div>
            </div>
            </ul>
          </li>
        </div>
      </div>
      </ul>
    </nav>
  </div>

  <div class="main">
    <video id="video" preload="auto" autoplay="true" loop="loop" muted="muted" volume="0">
      <source src="https://nohyoochan.github.io/img1/pf/pf1/homepage.mp4">
    </video>
    <div id="polina">
      <div class="text-box">
        <h1>오랜 시간 지속되도록 제작된 시계</h1>
        <p>내구성, 신뢰성, 지속성
        </p>
      </div>
    </div>
  </div>

  <div class="rc">
    <h2>Rolex Collection</h2>
  </div>
  <div class="swiper-container con">
    <div class="swiper-wrapper">
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w1.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>YACHT-MASTER</strong>
            <br>
            <span class="sp-text1">바다를 누비는 이들을 위한시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w2.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>DATEJUST</strong>
            <br>
            <span class="sp-text1">대표적인 클래식 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w3.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>GMT-MASTER II</strong>
            <br>
            <span class="sp-text1">코스모폴리탄을 위한 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w4.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>DAY-DATE</strong>
            <br>
            <span class="sp-text1">품격 있는 시계의 상징</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w5.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>SUBMARINER</strong>
            <br>
            <span class="sp-text1">다이버 워치의 대명사</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w6.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>OYSTER PERPETUAL</strong>
            <br>
            <span class="sp-text1">가장 기본적인 오이스터 모델</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w7.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>Cosmograph Daytona</strong>
            <br>
            <span class="sp-text1">레이싱을 위한 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w8.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>LADY-DATEJUST</strong>
            <br>
            <span class="sp-text1">클래식한 여성용 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w9.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>SEA-DWELLER</strong>
            <br>
            <span class="sp-text1">심해를 정복한 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w10.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>EXPLORER</strong>
            <br>
            <span class="sp-text1">새로운 기준을 세우다</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w11.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>SKY-DWELLER</strong>
            <br>
            <span class="sp-text1">세계 여행자들을 위한 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w12.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>MILGAUSS</strong>
            <br>
            <span class="sp-text1">과학자를 위한 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w13.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>CELLINI</strong>
            <br>
            <span class="sp-text1">클래식 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w14.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>PEARLMASTER</strong>
            <br>
            <span class="sp-text1">오이스터 컬렉션의 진주</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <div class="swiper-slide">
        <div class="inner">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/w15.png" alt="">
            </a>
          </div>
          <div class="name">
            <strong>AIR-KING</strong>
            <br>
            <span class="sp-text1">비행을 위한 시계</span>
            <br>
            <span class="sp-text2">$99,999</span>
          </div>
        </div>
      </div>
      <!-- Add Pagination -->
      <div class="swiper-scrollbar">
      </div>
    </div>
  </div>
  <div class="set-clock con">
    <h2>Set Clock</h2>
  </div>
  <div class="box-list-1 con">
    <div class="flex flex-row-wrap">
      <div class="box">
        <div class="content tab-type-1 con">
          <div class="head">
            <ul class="flex">
              <li class="flex-1-0-0 active" data-tab-name="box-1" data-tab-head-item-name="1"><a>클래식 시계</a></li>
              <li class="flex-1-0-0" data-tab-name="box-1" data-tab-head-item-name="2"><a>프로페셔널 시계</a></li>
              <li class="flex-1-0-0" data-tab-name="box-1" data-tab-head-item-name="3"><a>남성 시계</a></li>
              <li class="flex-1-0-0" data-tab-name="box-1" data-tab-head-item-name="4"><a>여성 시계</a></li>
              <li class="flex-1-0-0" data-tab-name="box-1" data-tab-head-item-name="5"><a>골드 시계</a></li>
            </ul>
          </div>
          <div class="body">
            <ul>
              <li class="active" data-tab-name="box-1" data-tab-body-item-name="1">
                <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-1.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Cellini</strong>
                    <br>
                    <span class="sp-text1">클래식 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-2.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust</strong>
                    <br>
                    <span class="sp-text1">클래식 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-3.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date</strong>
                    <br>
                    <span class="sp-text1">클래식 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-4.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Lady-Datejust</strong>
                    <br>
                    <span class="sp-text1">클래식 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-5.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Pearlmaster</strong>
            <br>
            <span class="sp-text1">클래식 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-6.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Oyster Perpetual</strong>
            <br>
            <span class="sp-text1">클래식 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-1-7.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Sky-Dweller</strong>
            <br>
            <span class="sp-text1">클래식 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        </div>
        
              </li>
              <li data-tab-name="box-1" data-tab-body-item-name="2">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-1.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>AIR-KING</strong>
                    <br>
                    <span class="sp-text1">프로페셔널 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-2.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Cosmograph Daytona</strong>
                    <br>
                    <span class="sp-text1">프로페셔널 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-3.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Explorer</strong>
                    <br>
                    <span class="sp-text1">프로페셔널 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-4.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>GMT-Master II</strong>
                    <br>
                    <span class="sp-text1">프로페셔널 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-5.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Sea-Dweller</strong>
            <br>
            <span class="sp-text1">프로페셔널 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-6.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Submariner</strong>
            <br>
            <span class="sp-text1">프로페셔널 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-7.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Yacht-Master</strong>
            <br>
            <span class="sp-text1">프로페셔널 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-2-8.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Milgauss</strong>
            <br>
            <span class="sp-text1">프로페셔널 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-1" data-tab-body-item-name="3">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-1.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Cosmograph Daytona</strong>
                    <br>
                    <span class="sp-text1">남성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-2.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust</strong>
                    <br>
                    <span class="sp-text1">남성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-3.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date</strong>
                    <br>
                    <span class="sp-text1">남성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-4.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Sky-Dweller</strong>
                    <br>
                    <span class="sp-text1">남성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-5.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Sea-Dweller</strong>
            <br>
            <span class="sp-text1">남성 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-6.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>GMT-Master II</strong>
            <br>
            <span class="sp-text1">남성 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-7.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Submariner</strong>
            <br>
            <span class="sp-text1">남성 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-3-8.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Yacht-Master</strong>
            <br>
            <span class="sp-text1">남성 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-1" data-tab-body-item-name="4">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/3-4-1.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust</strong>
                    <br>
                    <span class="sp-text1">여성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-4-2.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date</strong>
                    <br>
                    <span class="sp-text1">여성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-4-3.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Lady-Datejust</strong>
                    <br>
                    <span class="sp-text1">여성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-4-4.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Pearlmaster</strong>
                    <br>
                    <span class="sp-text1">여성 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-4-5.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Yacht-Master</strong>
            <br>
            <span class="sp-text1">여성 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-4-6.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Oyster Perpetual</strong>
            <br>
            <span class="sp-text1">여성 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-1" data-tab-body-item-name="5">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-1.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Cosmograph Daytona</strong>
                    <br>
                    <span class="sp-text1">골드 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-2.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust</strong>
                    <br>
                    <span class="sp-text1">골드 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-3.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date</strong>
                    <br>
                    <span class="sp-text1">골드 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-4.png" alt="" style="height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Lady-Datejust</strong>
                    <br>
                    <span class="sp-text1">골드 시계</span>
                    <br>
                    <span class="sp-text2">컬렉션 선택하기</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-5.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Sky-Dweller</strong>
            <br>
            <span class="sp-text1">골드 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-6.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Submariner</strong>
            <br>
            <span class="sp-text1">골드 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-7.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Yacht-Master</strong>
            <br>
            <span class="sp-text1">골드 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/3-5-8.png" alt="" style="height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Pearlmaster</strong>
            <br>
            <span class="sp-text1">골드 시계</span>
            <br>
            <span class="sp-text2">컬렉션 선택하기</span>
          </div>
        </div>
        </div>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  </div>
  </div>

  <div class="choose-clock con">
    <h2>Choose Clock</h2>
  </div>
  <div class="box-list-2 con">
    <div class="flex flex-row-wrap">
      <div class="box">
        <div class="content tab-type-2 con">
          <div class="head">
            <ul class="flex">
              <li class="flex-1-0-0 active" data-tab-name="box-2" data-tab-head-item-name="1"><a>남성용 시계</a></li>
              <li class="flex-1-0-0" data-tab-name="box-2" data-tab-head-item-name="2"><a>여성용 시계</a></li>
              <li class="flex-1-0-0" data-tab-name="box-2" data-tab-head-item-name="3"><a>오이스터스틸</a></li>
              <li class="flex-1-0-0" data-tab-name="box-2" data-tab-head-item-name="4"><a>골드 케이스</a></li>
              <li class="flex-1-0-0" data-tab-name="box-2" data-tab-head-item-name="5"><a>오이스터스틸과 골드</a></li>
              <li class="flex-1-0-0" data-tab-name="box-2" data-tab-head-item-name="6"><a>보석 세팅 베젤</a></li>
            </ul>
          </div>
          <div class="body">
            <ul>
              <li class="active" data-tab-name="box-2" data-tab-body-item-name="1">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-1.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Cosmograph Daytona</strong>
                    <br>
                    <span class="sp-text1">남성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 에버로즈 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-2.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Submariner Date</strong>
                    <br>
                    <span class="sp-text1">남성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 오이스터스틸과 옐로우 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-3-.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Yacht-Master II</strong>
                    <br>
                    <span class="sp-text1">남성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 44mm, 오이스터스틸과 에버로즈 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-4.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 41</strong>
                    <br>
                    <span class="sp-text1">남성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 41mm, 오이스터스틸과 옐로우 골드</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-5.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Sea-Dweller</strong>
            <br>
            <span class="sp-text1">남성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 43mm, 오이스터스틸과 옐로우 골드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-6.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>GMT-Master II</strong>
            <br>
            <span class="sp-text1">남성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 오이스터스틸과 에버로즈 골드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-7.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Pearlmaster 39</strong>
            <br>
            <span class="sp-text1">남성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 39mm, 화이트 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-1-8.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Cosmograph Daytona</strong>
            <br>
            <span class="sp-text1">남성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 옐로우 골드</span>
          </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-2" data-tab-body-item-name="2">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-1.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Lady-Datejust</strong>
                    <br>
                    <span class="sp-text1">여성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 28mm, 플래티넘과 다이아몬드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-2.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 36</strong>
                    <br>
                    <span class="sp-text1">여성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 오이스터스틸과 옐로우 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-3.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 36</strong>
                    <br>
                    <span class="sp-text1">여성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 에버로즈 골드와 다이아몬드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-4.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date 36</strong>
                    <br>
                    <span class="sp-text1">여성용 시계</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 옐로우 골드와 다이아몬드</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-5.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Day-Date 36</strong>
            <br>
            <span class="sp-text1">여성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 36mm, 화이트 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-6.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Datejust 31</strong>
            <br>
            <span class="sp-text1">여성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 31mm, 에버로즈 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-7.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Dayjust</strong>
            <br>
            <span class="sp-text1">여성용 시계</span>
            <br>
            <span class="sp-text2">오이스터, 31mm, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-2" data-tab-body-item-name="3">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-1.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Date 34</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸</span>
                    <br>
                    <span class="sp-text2">오이스터, 34mm, 오이스터스틸</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-2.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Milgauss</strong>
                    <br>
                    <span class="sp-text1"> 오이스터스틸</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 오이스터스틸</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-3.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Yacht-Master II</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸</span>
                    <br>
                    <span class="sp-text2">오이스터, 44mm, 오이스터스틸</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-4.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 36</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 화이트 골드와 다이아몬드</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-3-5.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Dayjust 41</strong>
            <br>
            <span class="sp-text1">오이스터스틸</span>
            <br>
            <span class="sp-text2">오이스터, 41mm, 오이스터스틸과 화이트 골드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-2-7.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Dayjust</strong>
            <br>
            <span class="sp-text1">오이스터스틸</span>
            <br>
            <span class="sp-text2">오이스터, 31mm, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
      
              </li>
              <li data-tab-name="box-2" data-tab-body-item-name="4">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-1.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Cosmograph Daytona</strong>
                    <br>
                    <span class="sp-text1">골드 케이스</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 옐로우 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-2.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date 36</strong>
                    <br>
                    <span class="sp-text1">골드 케이스</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 옐로우 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-3.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date 36</strong>
                    <br>
                    <span class="sp-text1">골드 케이스</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 옐로우 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-4.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date 36</strong>
                    <br>
                    <span class="sp-text1">골드 케이스</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 옐로우 골드와 다이아몬드</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-5.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Day-Date 40</strong>
            <br>
            <span class="sp-text1">골드 케이스</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 옐로우 골드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-6.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Day-Date 40</strong>
            <br>
            <span class="sp-text1">골드 케이스</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-7.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Datejust 31</strong>
            <br>
            <span class="sp-text1">골드 케이스</span>
            <br>
            <span class="sp-text2">오이스터, 31mm, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-4-8.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Lady-Datejust</strong>
            <br>
            <span class="sp-text1">골드 케이스</span>
            <br>
            <span class="sp-text2">오이스터, 28mm, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-2" data-tab-body-item-name="5">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/4-5-1.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Submariner Date</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸과 골드</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 오이스터스틸과 옐로우 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-5-2.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Yacht-Master II</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸과 골드</span>
                    <br>
                    <span class="sp-text2">오이스터, 44mm, 오이스터스틸과 에버로즈 골드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-5-3.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 36</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸과 골드</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 옐로우 골드와 다이아몬드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-5-4.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Sky-Dweller</strong>
                    <br>
                    <span class="sp-text1">오이스터스틸과 골드</span>
                    <br>
                    <span class="sp-text2">오이스터, 42mm, 오이스터스틸과 옐로우 골드</span>
                    </div>
                </div>
                </div>

        <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-5-6.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Yacht-Master 40</strong>
            <br>
            <span class="sp-text1">오이스터스틸과 골드</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 오이스터스틸과 에버로즈 골드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-5-7.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Submariner Date</strong>
            <br>
            <span class="sp-text1">오이스터스틸과 골드</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 오이스터스틸과 옐로우 골드</span>
          </div>
        </div>
        </div>
              </li>
              <li data-tab-name="box-2" data-tab-body-item-name="6">
              <div class="inner">
                  <div class="flex">
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                      <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-1.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 36</strong>
                    <br>
                    <span class="sp-text1">보석 세팅 베젤</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 화이트 골드와 다이아몬드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-2.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Datejust 36</strong>
                    <br>
                    <span class="sp-text1">보석 세팅 베젤</span>
                    <br>
                    <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 화이트 골드와 다이아몬드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-3.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date 40</strong>
                    <br>
                    <span class="sp-text1">보석 세팅 베젤</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 에버로즈 골드와 다이아몬드</span>
                    </div>
                  </div>
                  <div class="inner2">
                    <div class="img">
                    <a href="#">
                    <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-4.png" alt="" style="width:350px; height:350px;">
                    </a>
                    </div>
                    <div class="name">
                    <strong>Day-Date 40</strong>
                    <br>
                    <span class="sp-text1">보석 세팅 베젤</span>
                    <br>
                    <span class="sp-text2">오이스터, 40mm, 화이트 골드와 다이아몬드</span>
                    </div>
                </div>
                </div>

       <div class="flex">
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-5.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Day-Date 40</strong>
            <br>
            <span class="sp-text1">보석 세팅 베젤</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 화이트 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-6.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Day-Date 40</strong>
            <br>
            <span class="sp-text1">보석 세팅 베젤</span>
            <br>
            <span class="sp-text2">오이스터, 40mm, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-7.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Datejust</strong>
            <br>
            <span class="sp-text1">보석 세팅 베젤</span>
            <br>
            <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        <div class="inner3">
          <div class="img">
            <a href="#">
              <img src="https://nohyoochan.github.io/img1/pf/pf1/4-6-8.png" alt="" style="width:350px; height:350px;">
            </a>
          </div>
          <div class="name">
            <strong>Datejust 36</strong>
            <br>
            <span class="sp-text1">보석 세팅 베젤</span>
            <br>
            <span class="sp-text2">오이스터, 36mm, 오이스터스틸, 옐로우 골드와 다이아몬드</span>
          </div>
        </div>
        </div>
              </li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  </div>
  </div>

  <div class="page5">
    <h2>Learn more about Rolex</h2>
    <div class="main-bar">
      <div class="img-box con">
        <a href="#">
          <img src="https://nohyoochan.github.io/img1/pf/pf1/5-1.png" alt="">
        </a>
      </div>
      <div class="text-box-1 con">
        <a href="#">유서 깊은 탁월함
          <br>롤렉스는 모든 과정에 신중을 기하고 완벽함을 추구합니다.</a>
        <br>
        <p>롤렉스의 전문 기술</p>
        <pre>롤렉스는 시계의 바탕이 되는 전문 기술들을 계속해서 발전시켜 나가며      </pre>
        <pre>여러 세대를 거쳐 전수해왔습니다.</pre>
        <pre>전통 기법에서부터 첨단 기술에 이르기까지,</pre>
        <pre>롤렉스 시계에 담긴 노하우를 자세히 만나보세요.</pre>
      </div>
      <div class="img-box">
        <a href="#">
          <img src="https://nohyoochan.github.io/img1/pf/pf1/5-2.png" alt="">
        </a>
      </div>
      <div class="text-box-2">
        <a href="#">롤렉스의 중심이 되는 워치메이커</a>
        <p>롤렉스의 워치메이커들은 타임피스가 정상적으로
          <br>작동할수 있도록 세심하게 관리하는 전문가입니다.
          <br>초기 설계부터 제조뿐만 아니라 수리와 관리까지
          <br>모든 공정 단계에 참여합니다.</p>
      </div>
      <div class="text-box-2-2">
        <p>워치 메이킹</p>
        <pre>워치메이커들은 롤렉스 시계에 완벽함과 끊임없는
          <br>혁신을 추구하는 워치메이킹 정신을 담아 오래도록
          <br> 사용할 수 있는 특별한 시계를 완성합니다.</pre>
      </div>
      <div class="img-box">
        <a href="#">
          <img src="https://nohyoochan.github.io/img1/pf/pf1/5-3.png" alt="">
        </a>
      </div>
      <div class="text-box-3">
        <a href="#">지구를 지키기 위한 프로젝트</a>
        <p>영감을 주는 이야기</p>
      </div>
    </div>
  </div>

  <div id="map"></div>
  <div class="map-text">
    <a href="#">공식 판매점 찾기</a>
  </div>
  </div>
  </div>

  <div class="footer">
    <div class="footer-bar con">
      <div class="first">
        <div class="rolex-collection">
          <div class="oo">롤렉스 컬렉션</div>
          <ul>
            <li><a href="#">Rolex Collection</a></li>
            <li><a href="#">Set Clock</a></li>
            <li><a href="#">Choose Rolex</a></li>
            <li><a href="#">MAN Clock</a></li>
            <li><a href="#">WOMEN Clock</a></li>
          </ul>
        </div>
        <div class="rolex-world">
          <div class="oo">롤렉스 세계</div>
          <ul>
            <li><a href="#">Learn more about Rolex</a></li>
          </ul>
        </div>
        <div class="service">
          <div class="oo">서비스</div>
          <ul>
            <li><a href="#">공식 판매점 찾기</a></li>
            <li><a href="#">시계 관리 & 서비스</a></li>
            <li><a href="#">나의 컬렉션</a></li>
            <li><a href="#">자주 묻는 질문</a></li>
          </ul>
        </div>
      </div>
      <div class="second">
        <div class="collection">
          <div class="oo">컬렉션</div>
          <ul>
            <li><a href="#">AIR-KING</a></li>
            <li><a href="#">COSMOGRAPH DAYTONA</a></li>
            <li><a href="#">DATEJUST</a></li>
            <li><a href="#">LADY-DATEJUST</a></li>
            <li><a href="#">DAY-DATE</a></li>
            <li><a href="#">EXPLORER</a></li>
            <li><a href="#">GMT-Mater||</a></li>
            <li><a href="#">MILGAUSS</a></li>
            <li><a href="#">OYSTER PERPETUAL</a></li>
            <li><a href="#">PEARLMASTER</a></li>
            <li><a href="#">SEA-DWELLER</a></li>
            <li><a href="#">SKY-DWELLER</a></li>
            <li><a href="#">SUBMARINER</a></li>
            <li><a href="#">YACHT-MASTER</a></li>
            <li><a href="#">CELLINI</a></li>
          </ul>
        </div>
      </div>
      <div class="three">
        <div class="channel">
          <div class="oo">공식 채널</div>
          <ul>
            <li>
              <div class="facebook-bar">
                <span name="facebook" class="face">
                  <svg aria-labelledby="" height="15" width="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg"
                    role="img" class="footer-icon">
                    <path
                      d="m11.5 0v2.25h-2.93c-.31 0-.57.32-.57.63v1.62h3.25l-.61 2.25h-2.64v8.25h-2.25v-8.25h-2.25v-2.25h2.25v-1.75c0-1.59 1.17-2.75 2.75-2.75z">
                    </path>
                  </svg>
                </span>
                <a href="#" class="link" rel="noopener noreferrer" aria-label="페이스북" title="">페이스북</a></div>
            </li>
            <li>
              <div class="insta-bar">
                <span name="insta" class="sta">
                  <svg aria-labelledby="" height="15" width="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg"
                    role="img" class="footer-icon">
                    <path
                      d="m10.86 0h-6.72c-2.28 0-4.14 1.86-4.14 4.14v6.72c0 2.28 1.86 4.14 4.14 4.14h6.72c2.28 0 4.14-1.86 4.14-4.14v-6.72c0-2.28-1.86-4.14-4.14-4.14zm0 13.67h-6.72c-1.55 0-2.81-1.26-2.81-2.81v-6.72c0-1.55 1.26-2.81 2.81-2.81h6.72c1.55 0 2.81 1.26 2.81 2.81v6.72c0 1.55-1.26 2.81-2.81 2.81zm-3.36-10.13c-2.13 0-3.87 1.73-3.87 3.87 0 2.13 1.73 3.86 3.87 3.86 2.13 0 3.87-1.73 3.87-3.86 0-2.14-1.74-3.87-3.87-3.87zm0 6.4c-1.4 0-2.53-1.14-2.53-2.53 0-1.4 1.14-2.53 2.53-2.53s2.53 1.14 2.53 2.53-1.13 2.53-2.53 2.53zm4.72-7.25c.18.18.29.43.29.69s-.1.51-.29.69c-.18.18-.43.29-.69.29s-.51-.1-.69-.29c-.18-.18-.29-.43-.29-.69s.1-.51.29-.69c.18-.18.43-.29.69-.29.25.01.51.11.69.29z">
                    </path>
                  </svg>
                </span>
                <a href="#" class="link" rel="noopener noreferrer" aria-label="인스타그램" title="">인스타그램</a></div>
            </li>
            <li>
              <div class="twitter-bar">
                <span name="twitter" class="twi">
                  <svg aria-labelledby="" height="15" width="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg"
                    role="img" class="footer-icon">
                    <path
                      d="m15 2.85c-.55.24-1.14.41-1.77.49.64-.38 1.12-.98 1.35-1.7-.6.35-1.25.61-1.96.75-.56-.6-1.36-.97-2.24-.97-1.7 0-3.08 1.38-3.08 3.08 0 .24.03.48.08.7-2.55-.14-4.82-1.37-6.34-3.23-.26.45-.41.98-.41 1.55 0 1.07.54 2.01 1.37 2.56-.51-.02-.98-.16-1.4-.39v.04c0 1.49 1.06 2.74 2.47 3.02-.26.07-.53.1-.81.1-.2 0-.39-.02-.58-.05.39 1.22 1.53 2.11 2.87 2.14-1.05.82-2.38 1.31-3.82 1.31-.25 0-.49-.01-.73-.04 1.36.87 2.98 1.38 4.72 1.38 5.66 0 8.75-4.69 8.75-8.75l-.01-.4c.61-.43 1.13-.97 1.54-1.59zm0 0">
                    </path>
                  </svg>
                </span>
                <a href="#" class="link" rel="noopener noreferrer" aria-label="트위터" title="">트위터
                </a>
              </div>
            </li>
            <li>
              <div class="youtube-bar">
                <span name="youtube" class="you">
                  <svg aria-labelledby="" height="15" width="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg"
                    role="img" class="footer-icon">
                    <path
                      d="m12.19 1.88h-9.38c-1.54 0-2.81 1.26-2.81 2.81v5.62c0 1.55 1.27 2.81 2.81 2.81h9.37c1.55 0 2.81-1.27 2.81-2.81v-5.62c.01-1.55-1.26-2.81-2.8-2.81zm-6.57 9.37v-7.5l4.69 3.75z">
                    </path>
                  </svg>
                </span>
                <a href="#" class="link" rel="noopener noreferrer" aria-label="유튜브" title="">유튜브
                </a>
              </div>
            </li>
            <li>
              <div class="pinter-bar">
                <span name="pinter" class="pin">
                  <svg aria-labelledby="" height="15" width="15" viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg"
                    role="img" class="footer-icon">
                    <path
                      d="m13.5 5.34c0 3.18-2.04 5.75-4.87 5.75-.95 0-1.85-.49-2.15-1.06 0 0-.47 1.76-.59 2.2-.19.71-.65 1.57-1.02 2.18-.03.05-.06.11-.1.16-.02.03-.04.05-.05.08-.12.15-.25.29-.39.35 0 0-.16-.13-.25-.58 0-.03-.02-.07-.02-.1-.01-.09-.02-.19-.03-.3 0 0 0 0 0-.01-.06-.7-.08-1.58.07-2.24.17-.72 1.1-4.59 1.1-4.59s-.27-.56-.27-1.38c0-1.29.76-2.24 1.7-2.24.8 0 1.19.59 1.19 1.3 0 .79-.51 1.98-.78 3.08-.22.92.47 1.67 1.39 1.67 1.67 0 2.95-1.73 2.95-4.23 0-2.21-1.62-3.76-3.92-3.76-2.67 0-4.24 1.97-4.24 4.01 0 .79.31 1.65.7 2.11.07.09.08.17.06.26-.07.29-.23.92-.26 1.05-.04.17-.13.2-.31.12-1.18-.54-1.91-2.22-1.91-3.58 0-2.91 2.15-5.59 6.21-5.59 3.26 0 5.79 2.28 5.79 5.34z">
                    </path>
                  </svg>
                </span>
                <a href="#" class="link" rel="noopener noreferrer" aria-label="핀터레스트" title="">핀터레스트
                </a>
              </div>
            </li>
          </ul>
        </div>
        <div class="gozi">
          <div class="oo">법적 고지</div>
          <ul>
            <li><a href="#">이용약관</a></li>
            <li><a href="#">개인 정보 고지</a></li>
            <li><a href="#">쿠키</a></li>
          </ul>
        </div>
        <div class="media">
          <div class="oo">미디어</div>
          <ul>
            <li><a href="#">바탕화면</a></li>
            <li><a href="#">브로셔</a></li>
            <li><a href="#">사용자가이드</a></li>
          </ul>
        </div>
      </div>
    </div>
  </div>
</body>